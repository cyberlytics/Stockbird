...
def _import_data(input_path: Path, user_name: str = '', user_follower: int = 0, user_verified: bool = False,
    ...
    data = ap.import_data(input_path=input_path,
                          use_cols=['content', 'date', TweetColumns.RETWEETS.value],
                          drop_cols=['id', 'link', 'retweets', 'favorites', 'mentions', 'hashtags'],
...
def main():
    ...
    stockbird_logger.get_logger(LOGGER_NAME).info(f'Parser three started with following arguments: {args}')
