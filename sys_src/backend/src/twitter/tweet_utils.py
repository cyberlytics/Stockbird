...
import pandas as pd
from backend.src.Constants import TweetColumns, DEST_PATH, TWEETS_FILENAME
...
def query_tweets_by_substring(tweets_file_path: Path, sub_string: str):
    ...
    df_json = df.to_json(DEST_PATH / 'tweets_by_substring.json', orient='split', index=False, indent=4)
    return df_json
